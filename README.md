...
a `fast-check` property test per module (`<module>.property.test.js`, beside its
example test) that asserts the invariants across the whole input space. The `validate.sh` gate runs the same suite through
